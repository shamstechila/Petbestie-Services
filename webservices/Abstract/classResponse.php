...
class Response_Methods {
	...
	public function login_success($USERNAME,$PASSWORD)
		{
			...
			//$dataQueryInfo = "SELECT u.user_id,u.user_name_f,u.description_f,u.profile_id_f,u.email_f, u.profile_pic_f,u.pet_type_f,u.pet_name_f,u.pet_dob_f,u.species_f FROM user_details_t u, login_t l WHERE (u.user_id=l.user_id_fk) and (l.user_name_f= '$USERNAME' AND BINARY l.password_f='$PASSWORD')";
                                        ...
                                        $dataQueryInfo = "SELECT u.user_id,u.user_name_f,u.description_f,u.profile_id_f,u.email_f, u.profile_pic_f,u.pet_type_f,u.pet_name_f,u.pet_dob_f,u.species_f,u.notify_events_f FROM user_details_t u, login_t l WHERE (u.user_id=l.user_id_fk) and ((l.user_name_f= '$USERNAME' OR u.email_f= '$USERNAME') AND BINARY l.password_f='$PASSWORD')";
			...
			if(mysql_num_rows($dataResultSet) > 0)
			{
					...
					{														
						...
						$profile_pic=$row['profile_pic_f'];
                                                $notifyEvents=$row['notify_events_f'];
						...
						*/
						...
						$getGrpDetails['notifyEvents']=$notifyEvents;	
	...
	{
		$getList=array();
		$dataQueryInfo = "SELECT user_id,user_name_f,profile_pic_f,pet_name_f,pet_type_f,pet_dob_f,description_f,species_f,notify_events_f FROM user_details_t WHERE user_id='$lastInsertId'";
		$dataResultSet = mysql_query($dataQueryInfo,$GLOBALS['link']);
		$rm=new Response_Methods();
		$followArray=array();
		$getGroupDetails=array();
			...
			if(mysql_num_rows($dataResultSet) > 0){
				...
				while($row=mysql_fetch_array($dataResultSet)){
					...
					$getGroupDetails['pet_bio']=$rm->removeslashes($row['description_f']);	
                                        $getGroupDetails['species']=$row['species_f'];	
                                        $getGroupDetails['notifyEvents']=$row['notify_events_f'];	                                    	
					//echo 'test';
	...
	{
				$errorCode = "2";
				$errorMsg = "Username Already Exists. Please choose other one.";
				$newData = "{\"data\":{\"Error_Code\":\"".$errorCode."\",\"Error_Msg\":\"".$errorMsg."\"}}";
				return $newData;
	}
	...
	public function emailExistJson() {
		...
		$errorCode = "3";
		$errorMsg = "Email Address Already Exist,Please Enter Another Email Address";
		$newData = "{\"data\":{\"Error_Code\":\"".$errorCode."\",\"Error_Msg\":\"".$errorMsg."\"}}"; //Json Format Response
		echo $newData;
	}
...
function getPosts($userid,$minPostId=0)
{
...
ORDER BY N.post_id DESC LIMIT $limit;";
}
else
{
$loadMore=0;
...
ORDER BY N.post_id DESC LIMIT $limit;";
			...
			$remainingPosts=$rm->remainingPosts($userid,$minPostId,$limit);
			...
			{			
					...
					{														
						...
						$profilePic=$row['profile_pic_f'];
                                                $imageWidth=$row['image_width_f'];
						$imageHeight=$row['image_height_f'];
						...
						$thumb5=$row['thumb5'];
                                                ...
                                                $thumb4_width=$row['ipad_portrait_width'];
						$thumb4_height=$row['ipad_portrait_height'];
						$thumb5_width=$row['ipad_landscape_width'];
						$thumb5_height=$row['ipad_landscape_height'];
						...
						$getGrpDetails['post_description']=$rm->removeslashes($post_description);
						...
						$getGrpDetails['profilePic']=$profilePic;
                                                $getGrpDetails['post_image_width']=$imageWidth;	
						$getGrpDetails['post_image_height']=$imageHeight;

                                                $getGrpDetailsImage['post_image_url_iphone4']=$thumb1;	
						$getGrpDetailsImage['post_image_url_iphone5']=$thumb1;
						$getGrpDetailsImage['post_image_url_iphone6']=$thumb2;	
						$getGrpDetailsImage['post_image_url_iphone6+']=$thumb3;	
						$getGrpDetailsImage['post_image_url_iPad_portrait']=$thumb4;	
						$getGrpDetailsImage['post_image_url_iPad_landscape']=$thumb5; 


                                                $getGrpDetailsImageDimension['ipad_portrait_width']=$thumb4_width;	
						$getGrpDetailsImageDimension['ipad_portrait_height']=$thumb4_height;

                                                $getGrpDetailsImageDimension['ipad_landscape_width']=$thumb5_width;	
						$getGrpDetailsImageDimension['ipad_landscape_height']=$thumb5_height;


                                                $getGrpDetails['post_photos_thumbs']=$getGrpDetailsImage; 

                                                $getGrpDetails['post_photo_sizes']=$getGrpDetailsImageDimension; 
				...
				$newData=str_replace('\/', '/', $newData);
				$newData=substr($newData,1,strlen($newData)-2);	
				$newData="{\"data\":{\"Error_Code\":\"1\",\"Error_Msg\":\"News Feed List\",\"followersCount\":\"$followersCount\",\"followingCount\":\"$followingCount\",\"RemainingFeeds\":\"$remainingPosts\",\"result\":".$newData."}}";					
				return $newData; 
			}	
			else
				{
...
function getPhotos($userid)
{
			...
			$sqlFetchPhotos="select post_id,post_image_f from news_feeds_t where user_id_fk=$userid and post_image_f is not null order by post_id desc";
...
/*
$sqlFetchAllFriends="SELECT F.status, U.user_name_f, U.profile_pic_f FROM user_details_t U, friends_t F WHERE
F.friend_one = $USERID OR F.friend_two= $USERID AND F.status='1'";*/
			...
			{			
				...
				$newData=str_replace('\/', '/', $newData);
				$newData=substr($newData,1,strlen($newData)-2);	
				$newData="{\"data\":{\"Error_Code\":\"62\",\"Error_Msg\":\"Friend List\",\"result\":".$newData."}}";					
				return $newData; 
			}	
			else
				{
...
public function getFriendList($USERID)
	{
		$sqlFetchAllFriends="SELECT F.status, U.user_name_f, U.profile_pic_f,U.user_id FROM user_details_t U, friends_t F WHERE
CASE WHEN F.friend_one = $USERID THEN F.friend_two = U.user_id WHEN F.friend_two= $USERID THEN F.friend_one= U.user_id END AND
F.status='1'"; 
/*
$sqlFetchAllFriends="SELECT distinct F.status, U.user_name_f, U.profile_pic_f FROM user_details_t U, friends_t F WHERE
F.friend_one = $USERID OR F.friend_two= $USERID AND F.status='1'";*/
...
public function checkExistingFollow($userId, $friendId)
	{
				 ...
				 //$sqlCheckFollow="SELECT follow_id FROM follow_t WHERE(follower_user_id_fk=$userId OR following_user_id_fk=$userId) AND (follower_user_id_fk=$friendId OR following_user_id_fk=$friendId)";
                                	...
                                	//$sqlCheckFollow="SELECT follow_id FROM follow_t WHERE(follower_user_id_fk=$userId OR following_user_id_fk=$friendId) AND (follower_user_id_fk=$userId OR following_user_id_fk=$friendId)";

$sqlCheckFollow="SELECT follow_id FROM follow_t WHERE(follower_user_id_fk=$userId) AND (following_user_id_fk=$friendId)";
...
N.post_id=$postId";
		...
		public function getLikedUserIds($userId)
		{
				...
				$sqlgetLikePosts="SELECT U.user_id, U.user_name_f, U.profile_pic_f, N.post_id,N.post_image_f, L.likeDate_f FROM likes_t L, user_details_t U, news_feeds_t N where U.user_id=L.user_id_fk and N.post_id=L.post_id_fk and L.user_owner_id_fk=$userId and L.like_status_f=1 and L.user_id_fk <> $userId order by L.like_id desc";
				//die();
				...
				while($row=mysql_fetch_array($dataResultSet))
					{	
						...
						$post_id=$row['post_id'];
						$post_image=$row['post_image_f'];
                                                $like_date=$row['likeDate_f'];
						
						
						$getGrpDetails["created"]=$like_date;
...
public function getCommentedUserIds($userId)
		{
				...
				//$sqlgetLikePosts="SELECT user_id_fk,post_id_fk FROM likes_t where post_id_fk IN(select post_id from news_feeds_t where user_id_fk=$userId) and like_status_f=1";
				 ...
				 $sqlgetLikePosts="SELECT U.user_id, U.user_name_f, U.profile_pic_f, N.post_id,N.post_image_f,C.comment_date,C.comment_id,C.comment_text_f,C.user_id_fk FROM comments_t C, user_details_t U, news_feeds_t N where U.user_id=C.user_id_fk and N.post_id=C.post_id_fk and C.user_owner_id_fk=$userId and C.comment_status_f=1 and C.user_id_fk <> $userId order by C.comment_id desc";
				//die();
				$dataResultSet = mysql_query($sqlgetLikePosts,$GLOBALS['link']);
				$countRows=mysql_num_rows($dataResultSet);
				if($countRows>0)
				...
				while($row=mysql_fetch_array($dataResultSet))
					{	
						...
						$commented_user=$row['user_id_fk'];
						$comment_date=$row['comment_date'];
						
						
						$getGrpDetails["created"]=$comment_date;
						$getGrpDetails['notificationType']="C";				
						$getGrpDetails["user_id"]=$user_id;
						$getGrpDetails["user_name"]=$user_name;
						$getGrpDetails["profile_pic"]=$profile_pic;
						...
						$getGrpDetails["comment"]=$comment_text;
						$getGrpDetails["commented_user"]=$commented_user;
...
public function notificationList($userId)
		{
				...
				$rm=new Response_Methods();
				$getArrayLiked=$rm->getLikedUserIds($userId);
$getArrayCommented=array();
				$getArrayCommented=$rm->getCommentedUserIds($userId); /* commented on 10th Oct 2015 as asked by Bhusan temporarily */
				//print_r($getArrayCommented);
				
				$arrayMergeLikeComments=array_merge($getArrayLiked,$getArrayCommented); 

				//print_r($arrayMergeLikeComments);
				  foreach ($arrayMergeLikeComments as $key => $part) {
						   $sort[$key] = strtotime($part['created']);
					  }
  				array_multisort($sort, SORT_DESC, $arrayMergeLikeComments);
				//usort($arrayMergeLikeComments,"cmp");
				$fullList=array_merge($getArrayList,$arrayMergeLikeComments);
				//echo count($fullList);
				//print_r($fullList);
				//die();
 $friendsListArray=array();

if($rm->getFriendList($userId)!=0) 
{                               
 $friendsListArray=$rm->getFriendList($userId);
}
//$fullList=array_merge($fullList,$friendsListArray);
				if(count($fullList)>0 || count($friendsListArray)>0)
				{
				 if(count($fullList)>0 && count($friendsListArray)>0)
                                  {
				...
				$newData=str_replace('\/', '/', $newData);
				$newData=substr($newData,1,strlen($newData)-2);	
$friendListData=json_encode(array($friendsListArray));
$friendListData=str_replace('\/', '/', $friendListData);
$friendListData=substr($friendListData,1,strlen($friendListData)-2);	
				
				$newData="{\"data\":{\"Error_Code\":\"1\",\"Error_Msg\":\"Notification List\",\"result\":".$newData.",\"friendListData\":".$friendListData."}}";
                                 }


				else if(count($fullList)>0)
                                  {
				$newData=json_encode(array($fullList));
				$newData=str_replace('\/', '/', $newData);
				$newData=substr($newData,1,strlen($newData)-2);	
				...
				$newData="{\"data\":{\"Error_Code\":\"1\",\"Error_Msg\":\"Notification List\",\"result\":".$newData."}}";
                                ...
                                else if(count($friendsListArray)>0)
                                  {
				$friendListData=json_encode(array($friendsListArray));
$friendListData=str_replace('\/', '/', $friendListData);
$friendListData=substr($friendListData,1,strlen($friendListData)-2);	
				
				$newData="{\"data\":{\"Error_Code\":\"1\",\"Error_Msg\":\"Notification List\",\"friendListData\":".$friendListData."}}";
                                 }
...
function getNextPhotos($userid,$postId)
{
			...
			$sqlFetchPhotos="select post_id,post_image_f from news_feeds_t where user_id_fk=$userid and post_image_f is not null and post_id > $postId order by post_id desc";
			$dataResultSet = mysql_query($sqlFetchPhotos,$GLOBALS['link']);
			$rm=new Response_Methods();
			$getGrpDetails=array();
			$getArrayList=array();
	...
	$dataQueryInfo = "SELECT u.user_id,u.user_name_f,c.comment_text_f,c.comment_date,c.comment_id FROM user_details_t u, comments_t c WHERE (u.user_id=c.user_id_fk) and (c.post_id_fk=$post_id) order by c.comment_id desc";
			...
			if(mysql_num_rows($dataResultSet) > 0)
			{
					...
					{														
						...
						$getGrpDetails['commentText']=$rm->removeslashes($commentText);	
...
public function deleteCommentSuccessJson($post_id)
{
 				$getArrayList=array();
			//$dataQueryInfo = "SELECT login_id,user_name_f FROM login_t WHERE user_name_f= '$USERNAME' AND password_f='$PASSWORD'";
			$dataQueryInfo = "SELECT u.user_id,u.user_name_f,c.comment_text_f,c.comment_date,c.comment_id FROM user_details_t u, comments_t c WHERE (u.user_id=c.user_id_fk) and (c.post_id_fk=$post_id) order by c.comment_id desc";
			...
			if(mysql_num_rows($dataResultSet) > 0)
			{
				...
				$newData=str_replace('\/', '/', $newData);
				$newData=substr($newData,1,strlen($newData)-2);	
				$newData="{\"data\":{\"Error_Code\":\"1\",\"Error_Msg\":\"Comment Deleted Successful\",\"result\":".$newData."}}";					
				return $newData; 
			}	
			else
				{
					$errorCode = "1";
					$errorMsg = "No Comments to display.";
					$newData = "{\"data\":{\"Error_Code\":\"".$errorCode."\",\"Error_Msg\":\"".$errorMsg."\"}}"; //Json Format Response	
					return $newData;
				 //Comment Unsuccessful
				}
...
public function getSingleUserDetails($userId)
{
		...
		//default for trending with maximum likes
	
		 $sqlFetchAllUsers="SELECT U.user_name_f, U.profile_pic_f, U.pet_name_f, U.description_f, U.pet_dob_f, U.species_f, U.pet_type_f FROM user_details_t U left join login_t L on L.user_id_fk = U.user_id where U.user_id=$userId";
		 $errorSuccessCode=1;
		//die('test');
//echo $sqlFetchAllUsers;
			...
			{			
					...
					{	
						...
						$user_name=$row['user_name_f'];
						$profile_pic=$row['profile_pic_f'];
						$pet_bio=$row['description_f'];
						
						$pet_dob=$row['pet_dob_f'];
						$species=$row['species_f'];
						$pet_type=$row['pet_type_f'];
						
						
						$getGrpDetails['pet_name']=$pet_name;	
						$getGrpDetails['user_name']=$user_name;	
						$getGrpDetails['profile_pic_url']=$profile_pic;	
						$getGrpDetails['pet_bio']=$pet_bio;
						
						$getGrpDetails['pet_dob']=$pet_dob;	
						$getGrpDetails['species']=$species;	
						$getGrpDetails['pet_type']=$pet_type;	
...
public function userEmailAvailable()
	{
				$errorCode = "1";
				$errorMsg = "username and emailid available.";
				$newData = "{\"data\":{\"Error_Code\":\"".$errorCode."\",\"Error_Msg\":\"".$errorMsg."\"}}";
				return $newData;
	}
 
public function userUpdateProfileFail()
{
 				$errorCode = "0";
				$errorMsg = "No Changes made in Profile.";
				$newData = "{\"data\":{\"Error_Code\":\"".$errorCode."\",\"Error_Msg\":\"".$errorMsg."\"}}";
				return $newData;
}
			...
			{			
					...
					if($aboutUser)      
                        ...
                        $getGrpDetails['about_user']="Hi User, This is just a dummy description"; //setting default description text           
						 ...
						 $checkFriend=$rm->checkExistingFriendShip($loggedInUserId, $USERID); //checking friend relation
							if($checkFriend==0)
							{
							$getGrpDetails['friend_status']="-1";
							}
						...
						$checkFollow=$rm->checkExistingFollow($loggedInUserId, $USERID);; //check whether any follow relationship exists between them
						if($checkFollow>0)
						{
						$getGrpDetails['follow_status']="1";		
						}
...
public function getImageDimensions($data)
{
$src = imagecreatefromstring($data);				
$width = imagesx($src); 
$height = imagesy($src);
...
public function remainingPosts($userid,$minPostId,$limit)
{
//echo $limit;
if($minPostId!=0)
{
$sqlFetchPosts="SELECT N.post_id
FROM user_details_t U, news_feeds_t N, friends_t F
WHERE 
N.user_id_fk = U.user_id
AND 
...
{
$sqlFetchPosts="SELECT N.post_id, N.user_id_fk, U.user_id, F.friend_one,F.friend_two
FROM user_details_t U, news_feeds_t N, friends_t F
WHERE 
N.user_id_fk = U.user_id
AND 
...
//echo $sqlFetchPosts;
$dataResultSetCount = mysql_query($sqlFetchPosts,$GLOBALS['link']);
$totNewsfeeds=mysql_num_rows($dataResultSetCount);
$remainingNewsFeeds=$totNewsfeeds-$limit;
if($remainingNewsFeeds<0)
$remainingNewsFeeds=0;

//echo $remainingNewsFeeds;
return $remainingNewsFeeds;
}



function changeCommentStatusSuccess()
{
 				$errorCode = "21";
				$errorMsg = "Comment Status Changed.";
				$newData = "{\"data\":{\"Error_Code\":\"".$errorCode."\",\"Error_Msg\":\"".$errorMsg."\"}}";
				return $newData;
}	


function changeCommentStatusFail()
{
 				$errorCode = "23";
				$errorMsg = "Comment Status Not Changed.";
				$newData = "{\"data\":{\"Error_Code\":\"".$errorCode."\",\"Error_Msg\":\"".$errorMsg."\"}}";
				return $newData;
}
...
$dataQueryInfo = "SELECT u.user_id,u.user_name_f,u.profile_pic_f,c.comment_text_f,c.comment_date,c.comment_id,n.post_id,n.post_description_f,n.post_image_f,n.post_date_f,n.like_count_f,n.image_width_f,n.image_height_f FROM user_details_t u, comments_t c,news_feeds_t n WHERE (u.user_id=c.user_id_fk) and (n.post_id=c.post_id_fk) and (c.post_id_fk=$postId) order by c.comment_id desc";
 ...
 $postUserId=$rm->idToValue('user_id_fk','news_feeds_t','post_Id',$postId);
                ...
                $postUserName=$rm->idToValue('user_name_f','user_details_t','user_id',$postUserId); //getting user details
$user_profile_pic=$rm->idToValue('profile_pic_f','user_details_t','user_id',$postUserId); //getting user details
...
//$deviceToken = '8de61427b90700c906cc72b8ce1c71515e7f4f156bcee2ed52d10e7fdb13ea98';
//$deviceToken = 'b91d10f3a7b971d1a8be7beb0c7b36680ca9c971f02e73287cbf1dded952d948';

//$deviceToken = '1d01869a3aecf1f45e22f3fa5b67b1c58f646cf37fdd13b46857bf1687353d7b';
...
public function createThumbs($data,$userImageBaseURL,$image,$process)
{
//$IMAGEURLBASEURL=BASEURL.'/images/';
echo $IMAGEURLBASEURL=BASEURL.'/';
if($process=='signup')
$userImageBaseURL=$userImageBaseURL.'/profile_pics';
else if($process=='news_feed_post')
$userImageBaseURL=$userImageBaseURL.'/post_photos';

$src = imagecreatefromstring($data);				
$width = imagesx($src); 
$height = imagesy($src);
$widthArray=array(320,375,414,768,1024);

$thumbArray=array();
for($i=0;$i<count($widthArray);$i++)
{

$newwidth=$widthArray[$i];
//$newheight=80;
$newheight=($height/$width)*$newwidth;
$tmp=imagecreatetruecolor($newwidth,$newheight);
imagecopyresampled($tmp,$src,0,0,0,0,$newwidth,$newheight,$width,$height);
$filename = $userImageBaseURL.'/thumb1'.($i+1).$image;
imagejpeg($tmp,$filename,100);
imagedestroy($tmp);
$thumbNames['filename']=$IMAGEURLBASEURL.$filename;
$thumbNames['width']=intval($newwidth);
$thumbNames['height']=intval($newheight);
array_push($thumbArray,$thumbNames);
}
imagedestroy($src);
return $thumbArray;
}
